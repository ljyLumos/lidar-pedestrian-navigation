...
class JRDBHandle:
    def __init__(self, split, cfg, sequences=None, exclude_sequences=None):
        ...
        # 获取检测器类型配置，默认使用 maskrcnn
        self._detector_type = cfg.get("detector_type", "maskrcnn")
        ...
        for seq_idx, seq_name in enumerate(self.sequence_names):
            self.sequence_handle.append(_SequenceHandle(
                self.data_dir, seq_name, detector_type=self._detector_type
            ))
...
class _SequenceHandle:
    def __init__(self, data_dir, sequence, use_unlabeled_frames=False, detector_type="maskrcnn"):
        """
        Args:
            data_dir: 数据目录
            sequence: 序列名称
            use_unlabeled_frames: 是否使用无标签帧
            detector_type: 2D检测器类型，可选:
                - "maskrcnn": 使用原始 Mask R-CNN 检测结果 (默认)
                - "yolov8": 使用 YOLOv8 检测结果
        """
        ...
        # load 2D detection - 根据检测器类型选择目录
        if detector_type == "yolov8":
            im_det_dir = os.path.join(data_dir, "detections", "detections_2d_stitched_yolov8")
        elif detector_type == "fused":
            im_det_dir = os.path.join(data_dir, "detections", "detections_2d_stitched_fused")
        else:  # default to maskrcnn
        ...
        if os.path.exists(fname):
            with open(fname, "r") as f:
                ...
                Each detection is a dict with keys:
                        box (list) (x0, y0, w, h) or (x1, y1, x2, y2) for YOLOv8
                    file_id (str)
            ...
            print(f"警告: 未找到 {detector_type} 检测结果文件: {fname}")
            print(f"将使用空检测结果。请先运行 generate_yolov8_detections.py 生成检测结果。")
            self.im_dets = {}
