...
def generate_pseudo_labels(
    ...
    Note:
        If boxes is empty or 1-dimensional, returns empty pseudo labels and
        all-True negative mask (all points can be used as negatives).
    ...
    """
    # Handle empty or invalid boxes array
    if boxes is None or len(boxes) == 0 or boxes.ndim != 2 or boxes.shape[1] != 4:
        # No valid boxes: return empty pseudo labels and all-True negative mask
        num_laser_pts = len(laser_r)
        return (
            np.zeros((0, 2), dtype=np.float32),  # empty pl_loc_xy
            np.zeros((0, 4), dtype=np.float32),  # empty pl_boxes
            np.ones(num_laser_pts, dtype=bool),  # all points are negatives
        )
